...
// SendInteractiveCardToChat sends an interactive card message to a chat.
// POST /open-apis/im/v1/messages?receive_id_type=chat_id with msg_type=interactive
func (c *Client) SendInteractiveCardToChat(ctx context.Context, tenantToken, chatID string, card map[string]any) error {
	if tenantToken == "" {
		return errors.New("missing tenant token")
	}
	ep := strings.TrimRight(c.base(), "/") + "/open-apis/im/v1/messages?receive_id_type=chat_id"
	...
	defer resp.Body.Close()
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return errors.New("lark send interactive card status!=2xx")
	}
	return nil
}

// ReplyInteractiveCardInThread replies to a thread with an interactive card message.
// POST /open-apis/im/v1/messages/{message_id}/reply with msg_type=interactive
func (c *Client) ReplyInteractiveCardInThread(ctx context.Context, tenantToken, rootMessageID string, card map[string]any) error {
	if tenantToken == "" {
		return errors.New("missing tenant token")
	}
	if rootMessageID == "" {
	...
	defer resp.Body.Close()
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return errors.New("lark reply interactive card status!=2xx")
	}
	return nil
}
