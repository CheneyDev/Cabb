		...
		case "update", "updated":
			if links, _ := h.db.ListCNBIssuesByPlaneIssue(ctx, planeIssueID); len(links) > 0 {
				...
				for _, lk := range links {
					...
					// Preflight log: which fields are being sent
			...
			for _, m := range mappings {
				if !m.SyncDirection.Valid || strings.ToLower(m.SyncDirection.String) != "bidirectional" {
					continue
				}
				if !labelSelectorMatch(m.LabelSelector.String, labels) {
					continue
				}
...
func dataGetAssigneeIDs(m map[string]any) []string {
	...
	// helper to append unique non-empty
	add := func(s string) {
