...
func (h *Handler) CNBIngestPR(c echo.Context) error {
    ...
    evt := p.Event
    if strings.TrimSpace(p.Action) != "" { evt = p.Action }
...
func (h *Handler) processCNBIssue(p cnbIssuePayload, deliveryID, sum string) {
    ...
    case "issue.open":
        ...
        if deliveryID != "" { _ = h.db.UpdateEventDeliveryStatus(ctx, "cnb.issue", deliveryID, "succeeded", nil) }
    case "issue.close":
        if id, _ := h.db.FindPlaneIssueByCNBIssue(ctx, p.Repo, p.IssueIID); id != "" {
    ...
    case "issue.update":
        if id, _ := h.db.FindPlaneIssueByCNBIssue(ctx, p.Repo, p.IssueIID); id != "" {
            ...
            if strings.TrimSpace(p.Title) != "" { patch["name"] = p.Title }
            if len(p.Labels) > 0 {
                if ids, _ := h.db.MapCNBLabelsToPlane(ctx, mapping.PlaneProjectID, p.Repo, p.Labels); len(ids) > 0 {
                    patch["labels"] = ids
                }
            ...
            if len(patch) > 0 { _ = cl.PatchIssue(ctx, token, slug, mapping.PlaneProjectID, id, patch) }
            if deliveryID != "" { _ = h.db.UpdateEventDeliveryStatus(ctx, "cnb.issue", deliveryID, "succeeded", nil) }
        }
    default:
...
func (h *Handler) processCNBPR(p cnbPRPayload, evt, deliveryID, sum string) {
    ...
    case "opened", "open", "ready_for_review":
        if prMap != nil && prMap.OpenedStateID.Valid { targetState = prMap.OpenedStateID.String }
    case "review_requested":
        if prMap != nil && prMap.ReviewRequestedStateID.Valid { targetState = prMap.ReviewRequestedStateID.String }
    case "approved":
        if prMap != nil && prMap.ApprovedStateID.Valid { targetState = prMap.ApprovedStateID.String }
    case "merged", "merge":
        if prMap != nil && prMap.MergedStateID.Valid { targetState = prMap.MergedStateID.String }
    case "closed", "close":
        if prMap != nil && prMap.ClosedStateID.Valid { targetState = prMap.ClosedStateID.String }
    }
    if targetState != "" {
    ...
    if deliveryID != "" { _ = h.db.UpdateEventDeliveryStatus(ctx, "cnb.pr", deliveryID, "succeeded", nil) }
}
...
func (h *Handler) processCNBBranch(p cnbBranchPayload, evt, deliveryID, sum string) {
    ...
    if err != nil || planeIssueID == "" { return }
    ...
    case "create", "created":
        ...
        if deliveryID != "" { _ = h.db.UpdateEventDeliveryStatus(ctx, "cnb.branch", deliveryID, "succeeded", nil) }
    case "delete", "deleted":
