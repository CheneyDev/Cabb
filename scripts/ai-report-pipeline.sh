...
try_opencode() {
  ...
  prompt+="【输出格式 - 极其重要】\n"
  prompt+="1. 必须严格按照上下文中提供的 JSON 模板结构输出，不得简化或修改任何字段结构\n"
  prompt+="2. 直接输出 JSON（不要输出 markdown 代码块标记）\n"
  prompt+="3. 模板中的每个字段都必须保留，特别是 members 必须是对象数组而非字符串数组\n\n"
  prompt+="可调整项（仅数量，不可改变结构）：\n"
  ...
  prompt+="- meta.time_range: 必须是对象格式 {\"start\": \"${start_ts}\", \"end\": \"${end_ts}\", \"timezone\": \"${TZ}\"}\n"
  prompt+="- summary: 概括所有仓库的整体进展\n"
  prompt+="- highlights: 提炼 3-5 个重要亮点，用 [display_name] 前缀标注来源（如 [Cabb 前端]）\n"
  prompt+="- repos: 每个仓库包含 slug、display_name、brief、commit_count、has_commits、impact 和 members 数组\n"
  prompt+="- repos[].brief: 该仓库本周期的简要工作描述\n"
  prompt+="- repos[].has_commits: 布尔值，是否有提交\n"
  prompt+="- repos[].impact: 该仓库工作的影响和价值\n"
  prompt+="- repos[].members: 对象数组，每个成员包含 {name: 姓名, commits: 提交次数, achievements: [具体工作内容列表]}\n"
  prompt+="- repos[].members[].achievements: 字符串数组，根据该成员的 commit message 总结其具体工作内容，每条简洁明了\n\n"
  prompt+="要求：\n"
  prompt+="- 【必须】输出 JSON 结构必须与模板完全一致，members 必须是包含 name/commits/achievements 的对象数组\n"
  prompt+="- 结论先行，语言简练\n"
